...
function jaCarregou() {
    ...
    video1.onloadeddata = function(){
      video1.onseeked = function(){
        if(video1.seekable.end(0) >= video1.duration-0.1){
          f1 = true;
        } else {
          video1.currentTime=video1.buffered.end(0); // Seek ahead to force more buffering
        }
      };
      video1.currentTime=0; // first seek to trigger the event
    };

    video2.onloadeddata = function(){
      video2.onseeked = function(){
        if(video2.seekable.end(0) >= video2.duration-0.1){
          f2 = true;
        } else {
          video2.currentTime=video1.buffered.end(0); // Seek ahead to force more buffering
        }
      };
      video2.currentTime=0; // first seek to trigger the event
    };
